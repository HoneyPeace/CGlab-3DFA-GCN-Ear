...
$PaconvExpName = "S2G_MainPAConv_OriginalXYZ3_Latent1344_MDS_Seed1_LM069Fix"
$PaconvTag = "mainpaconv_original_xyz3_latent1344_mds_seed1_lm069fix"
$PaconvPidPath = Join-Path $RepoDir "debug_outputs\original_paconv_lm069fix_seed1.pid"

$ResultRoot = Join-Path $WorkspaceRoot "results\OPXYZ_DP18_LM069Fix"
$ExpName = "S2G_DP18_LM069Fix_S1"
$DebugDir = Join-Path $RepoDir "debug_outputs"
$StatusPath = Join-Path $DebugDir "lm069fix_dp18_dir1_seed1.status.tsv"
$PidPath = Join-Path $DebugDir "lm069fix_dp18_dir1_seed1.pid"
...
$TorchExtDir = Join-Path $DebugDir "torch_ext_lm069fix_dp18_dir1_sm86"

$Tag = "dp18_latres_decur_dir1_lm069fix_off_s1"
$RunName = "DP18_LATRES_DECUR_DIR1_LM069FIX_OFF"
...
function Wait-ForPaconv {
    if (-not (Test-Path -LiteralPath $PaconvPidPath)) {
        throw "PAConv PID file not found: $PaconvPidPath"
    }
    $paconvPid = [int](Get-Content -LiteralPath $PaconvPidPath | Select-Object -First 1)
    Add-Status "wait" "START" "Waiting for LM069Fix PAConv PID=$paconvPid"
    while (Get-Process -Id $paconvPid -ErrorAction SilentlyContinue) {
        Start-Sleep -Seconds 60
    }
    Add-Status "wait" "DONE" "LM069Fix PAConv PID=$paconvPid exited"
}
