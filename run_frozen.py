...
@File: run_frozen.py
@Description: 
[Ablation Study 풀-오토메이션 파이프라인]
1. Stage 1 (PAConv): 베이스라인 모델 1회 학습 및 평가 (이미 있으면 자동 스킵)
2. Bridge: PAConv 가중치를 Pretrained 폴더로 자동 복사
3. Stage 2 (DeepPA): 4가지 논문 절제 연구 조합(Ours, No-Gate, No-Attn, Simple) 연속 학습 및 평가
'''
...
def get_latest_run(output_root, exp_name, tag=None, required_model=None):
    ...
    실패한 학습은 무시하고 완료된(모델 파일이 있는) 폴더만 찾습니다.
    """
...
if __name__ == "__main__":
    ...
    for arg in user_args:
        if skip_next:
            skip_next = False
            continue
        if arg in ["--model", "--use_direct_regression", "--model_epoch", "--run_id", "--user_tag", "--use_gate", "--use_spatial_attention"]:
            skip_next = True
            continue
    ...
    # =========================================================================
    # [PHASE 2] DeepPA Ablation 4종 세트 연속 실행
    # =========================================================================
    ablation_configs = [
        {"name": "Full (Ours)",  "tag": "Stage2_DeepPA_Full",   "gate": "True",  "attn": "True"},
        {"name": "No-Gate",      "tag": "Stage2_DeepPA_NoGate", "gate": "False", "attn": "True"},
        {"name": "No-Attention", "tag": "Stage2_DeepPA_NoAttn", "gate": "True",  "attn": "False"},
        {"name": "Simple Add",   "tag": "Stage2_DeepPA_Simple", "gate": "False", "attn": "False"},
    ]

    for config in ablation_configs:
        print(f"\n===============================================================")
        print(f" 🧪 [PHASE 2 ABLATION] Running Config: {config['name']}")
        print(f"    - Gate Fusion : {config['gate']}")
        print(f"    - Spatial Attn: {config['attn']}")
        print(f"===============================================================")
        ...
        if p2_dir:
            ...
            deeppa_train_cmd = [sys.executable, "train.py"] + filtered_args + [
                ...
                "--use_spatial_attention", config['attn']
            ]
        ...
        if excel_exists:
            ...
            deeppa_eval_cmd = [sys.executable, "eval.py"] + filtered_args + [
                ...
                "--user_tag", config['tag'],
                "--use_gate", config['gate'],             # eval.py 가중치 로드 에러 방지를 위해 반드시 주입
                "--use_spatial_attention", config['attn'] # eval.py 가중치 로드 에러 방지를 위해 반드시 주입
            ]
