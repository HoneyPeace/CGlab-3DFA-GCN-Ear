...
if __name__ == "__main__":
    ...
    keys_to_override = ['--use_warmup', '--user_tag', '--run_id']
    base_args = filter_args(raw_user_args, keys_to_override)
    
    print("===============================================================")
    print(" 🚀 [ABLATION PIPELINE START] 히트맵 선학습(Warm-up) 효과 비교 실험")
    print("===============================================================\n")

    # 🌟 비교할 두 가지 실험 세팅 (태그명, 웜업 사용 여부)
    experiments = [
        {"tag": "NoWarmup", "use_warmup": "False", "desc": "1. 선학습 끄기 (처음부터 전면 RLW)"},
        {"tag": "WithWarmup", "use_warmup": "True", "desc": "2. 선학습 켜기 (히트맵 정체 후 전면 RLW)"}
    ]

    for exp in experiments:
        tag = exp["tag"]
        use_warmup = exp["use_warmup"]
        desc = exp["desc"]
        ...
        train_cmd = [sys.executable, "train.py"] + base_args + ["--use_warmup", use_warmup, "--user_tag", tag]
    ...
    print("\n===============================================================")
    print(" 🎉 [ALL EXPERIMENTS COMPLETED] 두 가지 세팅의 실험 및 평가가 모두 성공적으로 종료되었습니다!")
    print(" 📂 결과 폴더에서 NoWarmup 과 WithWarmup 엑셀 파일을 비교해 보세요.")
    print("===============================================================")