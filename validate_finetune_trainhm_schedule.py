...
CHECKS = [
    {"epoch": 1, "ramp_start": None, "pa": 0.1, "main": 1.0, "geom": 0.0, "aux": 1.0},
    {"epoch": 30, "ramp_start": None, "pa": 0.1, "main": 1.0, "geom": 0.0, "aux": 1.0 / 30.0},
...
def make_controller():
    ...
    controller.finetune_aux_mode = "drop"
...
def main():
    ...
    print("[CHECK] Finetune train Main_HM plateau schedule")
    print("[CHECK] PAConv HM is fixed at 0.1")
    print("[CHECK] Aux drops over 30 epochs")
    ...
    for check in CHECKS:
        ...
        _, weights = controller.compute_loss("deeppa_finetune", check["epoch"] - 1, **losses)
        record = {
            "epoch": check["epoch"],
    ...
    save_path = output_dir / f"finetune_trainhm_schedule_{time.strftime('%Y%m%d_%H%M%S')}.json"
    save_path.write_text(json.dumps(records, indent=2), encoding="utf-8")
    print(f"[CHECK] Saved validation records: {save_path}")

    if failures:
        print("[FAIL] Finetune schedule check failed")
        for failure in failures:
            print(f"[FAIL] {failure}")
        raise SystemExit(1)
    print("[PASS] Finetune train Main_HM plateau schedule weights are consistent")
